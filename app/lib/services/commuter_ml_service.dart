...
class CommuterMlService {
  ...
  ModelEvaluation get evaluation =>
      _evaluation ??
      const ModelEvaluation(
          mae: 1.8, rmse: 2.4, crowdAccuracy: 0.81, macroF1: 0.79);
  ...
  DelayCrowdPrediction predict(OperationalSnapshot snapshot) {
    ...
    final confidence =
        (0.7 + (1 / (1 + exp(-delay / 8))) * 0.25).clamp(0.72, 0.95);
  ...
  List<RouteRecommendation> optimizeRoutes({
    ...
    for (final route in candidates) {
      final firstLine =
          route.steps.isNotEmpty ? route.steps.first.line : 'MRT Kajang';
      ...
      final transferPenalty =
          route.steps.where((s) => s.type == RouteStepType.transfer).length *
              1.8;
      final adjustedMinutes = route.totalDurationMinutes +
          pred.predictedDelayMinutes +
          crowdPenalty;
  ...
  List<_SyntheticSample> _generateSyntheticDataset(int size) {
    ...
    return List.generate(size, (i) {
      ...
      final temp =
          27 + sin(dt.hour / 24 * pi * 2) * 4 + _random.nextDouble() * 2;
      final event = _random.nextDouble() < 0.12;
      final peakBoost =
          (dt.hour >= 7 && dt.hour <= 9) || (dt.hour >= 17 && dt.hour <= 20)
              ? 3.8
              : 0.8;
      ...
      final delay = max(
        ...
        peakBoost +
            ...
            weatherBoost +
            eventBoost +
            _random.nextDouble() * 2.4 -
            4.8,
      );
      final crowdSignal = stationBoost +
          (peakBoost * 0.8) +
          (rain * 0.3) +
          (event ? 2.2 : 0.0) +
  ...
  List<double> _featureVector(OperationalSnapshot snapshot) {
    ...
    final isPeak =
        ((hour >= 7 && hour <= 9) || (hour >= 17 && hour <= 20)) ? 1.0 : 0.0;
    final isWeekend =
        snapshot.timestamp.weekday >= DateTime.saturday ? 1.0 : 0.0;
