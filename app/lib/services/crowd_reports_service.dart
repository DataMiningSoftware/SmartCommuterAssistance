...
class StopCrowdForecast {
  ...
  bool get isWeekend => dayOfWeek >= 5;
  bool get isClosedHours => CrowdReportsService.isClosedHoursSource(sourceType);
...
class CrowdReportsService {
  ...
  Future<Map<String, StopCrowdForecast>> _fetchForecastGridRemote({
    ...
    final daysOfWeek = <int>{
      for (final time in times) _dayOfWeek(time),
    }.toList();
    ...
    try {
      final remoteRows = await _client
          ...
          .select(
            'stop_id,forecast_hour,day_of_week,occupancy_level,expected_wait_minutes,eta_multiplier,source_type,updated_at',
          )
          .inFilter('stop_id', normalizedStopIds)
          .inFilter('forecast_hour', hours)
          .inFilter('day_of_week', daysOfWeek)
          .timeout(const Duration(seconds: 5));
      rows.addAll(
        remoteRows
            .whereType<Map>()
  ...
  Future<bool> submitTripFeedback({
    ...
    required String destStop,
    required int predictedMinutes,
    required int actualMinutes,
    int? crowdAtStart,
    int delaysReported = 0,
    int crowdsReported = 0,
    ...
    try {
      await _client.from('trip_feedback').insert({
        ...
        'predicted_min': predictedMinutes,
        'actual_min': actualMinutes,
        'deviation_min': actualMinutes - predictedMinutes,
        'crowd_at_start': crowdAtStart,
        'delays_reported': delaysReported,
        'crowds_reported': crowdsReported,
        'walk_distance_m': walkDistanceM,
        'weather': weather,
        'time_of_day': DateTime.now().hour,
        'day_of_week': DateTime.now().weekday == DateTime.sunday ? 6 : DateTime.now().weekday - 1,
        'is_weekend': DateTime.now().weekday == DateTime.saturday || DateTime.now().weekday == DateTime.sunday,
      }).timeout(const Duration(seconds: 5));
      return true;
    } catch (_) {
      return false;
  ...
  StopCrowdForecast? _toStopCrowdForecast(Map<String, dynamic> map) {
    ...
    final dowRaw = map['day_of_week'];
    final dayOfWeek = dowRaw is num
        ? dowRaw.toInt()
        : int.tryParse(dowRaw?.toString() ?? '') ?? 0;
  ...
  static String forecastKey({
    ...
    return '${stopId.trim().toUpperCase()}|$forecastHour|$dayOfWeek';
  }
  ...
  static String forecastKeyForTime({
    ...
    return forecastKey(
      ...
      dayOfWeek: time.weekday == DateTime.sunday ? 6 : time.weekday - 1,
    );
  }
  ...
  static int _dayOfWeek(DateTime time) {
    return time.weekday == DateTime.sunday ? 6 : time.weekday - 1;
  }
  ...
  static bool _isSystemClosed(DateTime time) {
    return !OperatingHoursService.isAnyLineRunning(at: time);
  }
