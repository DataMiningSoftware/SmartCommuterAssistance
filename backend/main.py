...
class TripFeedbackRequest(BaseModel):
    ...
    predicted_min: int
    actual_min: int
    crowd_at_start: Optional[int] = None
    delays_reported: int = 0
    crowds_reported: int = 0
...
def crowd_blend(
    ...
    v_hour = hour if hour is not None else now.hour
    v_is_weekend = is_weekend if is_weekend is not None else now.weekday() >= 5

    try:
        result = (
            crowd_service._get_supabase()
            .rpc(
        ...
        logging.getLogger("smart_commuter.crowd_blend").warning(
            "crowd blend fallback for %s: %s", stop_id, exc
        )
...
@app.post("/trip/feedback", response_model=TripFeedbackResponse)
def submit_trip_feedback(body: TripFeedbackRequest) -> TripFeedbackResponse:
    now = datetime.now(MALAYSIA_TZ)
    dow = now.weekday()
    try:
        crowd_service._get_supabase().table("trip_feedback").insert(
            {
                "route_id": body.route_id.upper(),
                "origin_stop": body.origin_stop.upper(),
                "dest_stop": body.dest_stop.upper(),
                "predicted_min": body.predicted_min,
                "actual_min": body.actual_min,
                "deviation_min": body.actual_min - body.predicted_min,
                "crowd_at_start": body.crowd_at_start,
                "delays_reported": body.delays_reported,
                "crowds_reported": body.crowds_reported,
...
def plan_trip(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="Invalid departure datetime. Use ISO 8601 format.",
        )
    network = load_network()
...
def load_network() -> dict:
    ...
    csv_path = repo_root / "scripts" / "train_stops_kl.csv"
