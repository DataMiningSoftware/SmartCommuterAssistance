...
Render settings when the service root is the repository root:
...
```powershell
docker build -t smart-commuter-backend -f backend/Dockerfile .
docker run --rm -p 8000:8000 --env-file .env smart-commuter-backend
```
