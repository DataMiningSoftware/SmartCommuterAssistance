...
def synthetic_ridership_df() -> pd.DataFrame:
    """Fallback DataFrame used when no ridership CSV is available.

    Yields a flat line-strength signal so the forecast builder can still run
    (used by CI/cron before real ridership data is provisioned).
    """
    return pd.DataFrame({"date": [pd.Timestamp("2026-04-21")]})
...
def build_forecast_rows(
    ...
    # One reference date per day of week (Python weekday(): Mon=0 .. Sun=6).
    reference_dates = [
    ...
    for stop in stops:
        ...
        for day_of_week in range(7):
            ...
            reference = reference_dates[day_of_week]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to ridership_headline.csv (optional; falls back to a flat trend)",
    )
    parser.add_argument(
        "--stops-csv",
        type=Path,
...
if __name__ == "__main__":
    ...
    if ridership_csv is not None and ridership_csv.exists():
        ridership_df = load_ridership(ridership_csv)
    else:
        if ridership_csv is not None:
            print(f"Warning: ridership csv not found ({ridership_csv}); using flat trend.")
        ridership_df = synthetic_ridership_df()
