...
# Day-of-week multipliers applied on top of peak-period base values.
# Friday peaks are lighter (WFH), Saturday afternoons are heavier (shopping).
DOW_PEAK_MULTIPLIER = {
    0: 1.0,  # Monday
    1: 1.0,  # Tuesday
    2: 1.0,  # Wednesday
    3: 1.0,  # Thursday
    4: 0.75,  # Friday — lighter, many WFH or leave early
    5: 0.6,  # Saturday — no commute crush, shopping peaks midday
    6: 0.4,  # Sunday — lightest
}

# Extra boost for weekend shopping hours at commercial stations.
DOW_WEEKEND_SHOPPING_BOOST = {
    5: 8.0,  # Saturday
    6: 4.0,  # Sunday
}
...
def _cyclical_dow(day_of_week: int) -> tuple[float, float]:
    rad = 2.0 * math.pi * day_of_week / 7.0
...
def build_feature_row(
    ...
    dow_sin, dow_cos = _cyclical_dow(day_of_week)
    return {
...
def estimate_occupancy_percent(
    ...
    dow_mult = DOW_PEAK_MULTIPLIER.get(day_of_week, 1.0)
    peak_adjust = {0: 0.0, 1: 8.0, 2: 22.0, 3: 34.0}.get(peak_period, 0.0)
    peak_adjust = peak_adjust * dow_mult

    # Weekend shopping boost at commercial stations.
    shopping_boost = DOW_WEEKEND_SHOPPING_BOOST.get(day_of_week, 0.0)
    if shopping_boost > 0 and profile.pressure_score >= 3 and 10 <= hour <= 21:
        peak_adjust = max(peak_adjust, shopping_boost)

    commute_bonus = 0.0
    if is_weekend == 0:
        scaled = dow_mult
        if peak_period >= 3:
            commute_bonus = station_pressure * 3.0 * scaled
        elif peak_period == 2:
            commute_bonus = station_pressure * 2.0 * scaled
        elif peak_period == 1:
            commute_bonus = station_pressure * 1.0 * scaled
