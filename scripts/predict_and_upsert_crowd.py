...
def upsert_predictions(
    ...
    level_to_percent = {1: 15.0, 2: 35.0, 3: 50.0, 4: 70.0, 5: 90.0}
    level_to_wait = {1: 2, 2: 4, 3: 6, 4: 8, 5: 10}
    level_to_multiplier = {1: 1.0, 2: 1.05, 3: 1.12, 4: 1.22, 5: 1.35}
    rows = [
        {
            "stop_id": stop_id,
            ...
            "occupancy_percent": level_to_percent.get(level, 50.0),
            "expected_wait_minutes": level_to_wait.get(level, 4),
            "eta_multiplier": level_to_multiplier.get(level, 1.1),
            "source_type": "predicted",
            "updated_at": now.isoformat(),
        }
        for stop_id, level in zip(selected_stop_ids, levels)
    ]
    client.table("crowd_forecast_hourly").upsert(
        rows, on_conflict="stop_id,forecast_hour,day_of_week"
    ).execute()
    print(f"Upserted predictions for {len(rows)} stations (day {day_of_week}).")
